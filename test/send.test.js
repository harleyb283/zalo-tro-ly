...
setThrottle({ minKhoangCachMs: 0, toiDaMoiPhut: 100000 });
...
test('A1 ★ offset tính trên chuỗi ĐÃ BỎ dấu markdown, không phải chuỗi gốc', () => {
  const kq = markdownToZalo('xin chào **anh Hải** nhé');
...
test('A2 ★ NHIỀU đoạn đậm trên một dòng — offset phải dồn đúng, không tích luỹ lệch', () => {
  const kq = markdownToZalo('**một** giữa **hai** cuối **ba**');
...
test('A3 ★ TIẾNG VIỆT CÓ DẤU — offset không lệch vì chữ có dấu', () => {
  const kq = markdownToZalo('Đã duyệt **báo giá** của khách rồi ạ');
...
test('A7 danh sách -> lst_1 / lst_2, dấu đầu dòng bị bỏ', () => {
  const kq = markdownToZalo('- mục một\n- mục hai\n1. thứ nhất\n2) thứ nhì');
...
test('A8 ★ "* mục" là DANH SÁCH, "*chữ*" là NGHIÊNG — không lẫn nhau', () => {
  const ds = markdownToZalo('* mục danh sách');
  ...
  const ng = markdownToZalo('đây là *nghiêng* nhé');
...
test('A9 gạch ngang + lồng nhau (đậm chứa nghiêng)', () => {
  const kq = markdownToZalo('~~bỏ~~ và **đậm *và nghiêng* nữa**');
...
test('A10 `code` — bỏ dấu backtick, KHÔNG tô (Zalo không có monospace)', () => {
  const kq = markdownToZalo('chạy `npm test` đi');
...
test('A11 ★ dấu mở KHÔNG có dấu đóng -> giữ NGUYÊN VĂN, không nuốt mất chữ', () => {
  for (const [vao, ra] of [
    ...
    assert.equal(markdownToZalo(vao).msg, ra, `nuốt mất chữ ở: ${vao}`);
  }
});

test('A12 dòng cảnh báo -> màu, và màu phủ ĐÚNG dòng đó', () => {
  const kq = markdownToZalo('bình thường\n🔴 nguy hiểm\n⚠️ lưu ý\n✅ xong');
...
test('A14 chuỗi rỗng / không phải chuỗi -> không nổ', () => {
  assert.deepEqual(markdownToZalo(''), { msg: '', styles: [] });
  assert.deepEqual(markdownToZalo(null), { msg: '', styles: [] });
  assert.deepEqual(markdownToZalo(undefined), { msg: '', styles: [] });
});
...
test('B1 dưới trần -> giữ nguyên, không đánh dấu cắt', () => {
  const kq = truncateSafely('ngắn thôi');
...
test('B3 trần đúng bằng độ dài -> KHÔNG cắt (không lệch 1 ký tự)', () => {
  ...
  assert.equal(truncateSafely(s, 100).daCat, false);
  assert.equal(truncateSafely(s, 99).daCat, true);
});
...
test('C1 sendToGroup gửi đúng ThreadType.Group, sendHostDm gửi ThreadType.User', async () => {
  const api = apiGia();
  await sendToGroup(api, '111', 'chào nhóm');
  await sendHostDm(api, 'dm-1', 'chào riêng');
...
test('C2 ★ msgId trả về là CHUỖI, không phải number (ID Zalo vượt MAX_SAFE_INTEGER)', async () => {
  ...
  const kq = await sendToGroup(api, '111', 'x');
...
test('C3 ★ ghi lại chính tin vừa gửi, đúng hình dạng TinChuanHoa', async () => {
  const api = apiGia();
  const daGhi = [];
  await sendToGroup(api, '111', '**xong** rồi ạ', { ghiLai: (t) => daGhi.push(t) });
...
test('C4 KHÔNG truyền ghiLai -> vẫn gửi được, nhưng phải KÊU ra stderr', async () => {
  ...
  try {
    await sendToGroup(apiGia(), '111', 'x');
  } finally {
...
test('C6 lỗi gửi -> cleanError, KHÔNG rò cookie trong thông điệp', async () => {
  ...
  await assert.rejects(() => sendToGroup(api, '111', 'x'), (e) => {
...
test('C7 tin RỖNG bị từ chối, chưa api nào được gọi', async () => {
  const api = apiGia();
  await assert.rejects(() => sendToGroup(api, '111', ''), /RỖNG/);
  await assert.rejects(() => sendToGroup(api, '111', '   \n  '), /RỖNG/);
  assert.equal(api.goi.length, 0);
});

test('C8 chưa có phiên Zalo -> lỗi đọc được, không nổ TypeError', async () => {
  await assert.rejects(() => sendToGroup({}, '111', 'x'), /Chưa có phiên Zalo/);
  await assert.rejects(() => sendToGroup(null, '111', 'x'), /Chưa có phiên Zalo/);
});

test('C9 thiếu chatId -> ném NGAY, không gửi mò', async () => {
  const api = apiGia();
  await assert.rejects(() => sendToGroup(api, null, 'x'), /Thiếu ID bắt buộc/);
  assert.equal(api.goi.length, 0);
});

test('C10 text quá dài -> gửi bản đã cắt và báo daCat', async () => {
  const api = apiGia();
  const kq = await sendToGroup(api, '111', 'z'.repeat(GIOI_HAN.DO_DAI_TIN_TOI_DA + 100));
  assert.equal(kq.daCat, true);
...
test('C11 styles được truyền sang sendMessage; không có style thì bỏ hẳn field', async () => {
  const api = apiGia();
  await sendToGroup(api, '111', '**đậm**');
  assert.ok(Array.isArray(api.goi[0].noiDung.styles));
  await sendToGroup(api, '111', 'trơn');
...
test('C13 sendImage thiếu nguồn ảnh -> ném', async () => {
  await assert.rejects(() => sendImage(apiGia(), '111', null, 'x'), /thiếu nguồn ảnh/);
});
...
test('D1 ★ giãn cách tối thiểu giữa 2 tin liên tiếp', async () => {
  const cu = setThrottle({ minKhoangCachMs: 120, toiDaMoiPhut: 100000 });
  resetThrottle();
  try {
    const api = apiGia();
    const t0 = Date.now();
    await sendToGroup(api, '111', 'một');
    await sendToGroup(api, '111', 'hai');
    await sendToGroup(api, '111', 'ba');
...
test('D2 ★ gọi SONG SONG cũng không lọt qua cổng cùng lúc', async () => {
  const cu = setThrottle({ minKhoangCachMs: 80, toiDaMoiPhut: 100000 });
  resetThrottle();
  try {
    const moc = [];
    const api = {
    ...
    await Promise.all([1, 2, 3].map((i) => sendToGroup(api, '111', `tin ${i}`)));
...
test('D3 giá trị MẶC ĐỊNH đủ chậm để không giống máy bắn tin', () => {
  // Đọc DEFAULT_THROTTLE chứ KHÔNG đọc THROTTLE: đầu file này đã nới
  ...
  assert.ok(DEFAULT_THROTTLE.minKhoangCachMs >= 1000, 'nhanh hơn 1 tin/giây là không còn giống người');
  assert.ok(DEFAULT_THROTTLE.toiDaMoiPhut <= 30);
  assert.equal(Object.isFrozen(DEFAULT_THROTTLE), true, 'mặc định phải đóng băng để không ai nới ngầm');
});
...
test('G1 ★ khớp ĐÚNG số Zalo thật trả về: pos=9, len=15', () => {
  const kq = buildMentions(CAU_THAT, [{ uid: BOT_THAT, ten: TEN_BOT }]);
...
test('G3 ★ đơn vị đếm là NFC/UTF-16 — NFD cho 16 chứ không phải 15', () => {
  ...
  const kq = buildMentions(CAU_THAT.normalize('NFD'), [{ uid: BOT_THAT, ten: TEN_BOT.normalize('NFD') }]);
...
test('G4 🔴 KHÔNG tra ra uid -> để nguyên chữ, TUYỆT ĐỐI không bịa uid', () => {
  const kq = buildMentions('nhờ @Người Lạ xem giúp', [{ uid: BOT_THAT, ten: TEN_BOT }]);
...
test('G5 🔴 TRÙNG TÊN -> không đoán, không tag ai, và BÁO ra', () => {
  ...
  const kq = buildMentions('@Hải xem giúp', ds);
...
test('G6 khớp THAM LAM tên dài nhất: "@Lan Anh" thắng "@Lan"', () => {
  ...
  const kq = buildMentions('@Lan Anh ơi', ds);
  assert.equal(kq.mentions.length, 1);
...
test('G8 KHÔNG tự đặt trường `type` — @All là quyết định của zca-js, không phải của ta', () => {
  ...
  const kq = buildMentions('chào @An', [{ uid: '111', ten: 'An' }]);
...
test('H1 có ghiLai -> câu trả lời vào kho, kèm uid bot tiêm từ ngoài', async () => {
  const api = apiGia();
  const ghi = [];
  setThrottle({ moiTinMs: 0, dmMs: 0 });
  await sendToGroup(api, '9995000000000000005', 'Dạ em xem rồi ạ', {
    ghiLai: (t) => ghi.push(t),
...
test('H2 KHÔNG có ghiLai -> phải KÊU, vì đây đúng là cách bug vừa rồi xảy ra', async () => {
  ...
  setThrottle({ moiTinMs: 0, dmMs: 0 });
  try {
    await sendToGroup(api, '111', 'xin chào');
  } finally {
    process.stderr.write = goc;
    resetThrottle();
  }
...
test('H3 mention chỉ đi vào NHÓM — trong DM zca-js bỏ hết, đừng gửi thừa', async () => {
  const api = apiGia();
  setThrottle({ moiTinMs: 0, dmMs: 0 });
  await sendHostDm(api, 'dm1', 'chào @An', { dsNguoi: [{ uid: '111', ten: 'An' }] });
  resetThrottle();
...
test('H4 gửi vào NHÓM có dsNguoi -> mentions đi kèm đúng khuôn zca-js', async () => {
  const api = apiGia();
  setThrottle({ moiTinMs: 0, dmMs: 0 });
  await sendToGroup(api, 'nhom1', 'nhờ @An xem giúp', { dsNguoi: [{ uid: '111', ten: 'An' }] });
  resetThrottle();
...
test('H5 tag người KHÔNG có trong nhóm -> gửi tin KHÔNG kèm mentions nào', async () => {
  const api = apiGia();
  setThrottle({ moiTinMs: 0, dmMs: 0 });
  await sendToGroup(api, 'nhom1', 'nhờ @Người Lạ xem', { dsNguoi: [{ uid: '111', ten: 'An' }] });
  resetThrottle();
...
test('I1 tin dài -> nhiều tin, gộp lại KHÔNG mất chữ nào', async () => {
  const api = apiGia();
  setThrottle({ moiTinMs: 0, dmMs: 0, minKhoangCachMs: 0, toiDaMoiPhut: 999 });
  const kq = await sendInParts(api, 'nhom1', RAT_DAI);
  resetThrottle();
...
test('I2 ★ CHỨNG MINH không cắt chồng: truncateSafely() là NO-OP trên mọi phần', async () => {
  const api = apiGia();
  setThrottle({ moiTinMs: 0, dmMs: 0, minKhoangCachMs: 0, toiDaMoiPhut: 999 });
  await sendInParts(api, 'nhom1', RAT_DAI);
  resetThrottle();
  for (const g of api.goi) {
    const m = g.noiDung.msg;
    assert.ok(m.length <= 4000, `phần dài ${m.length} > trần`);
    // Vân tay của truncateSafely() là hậu tố này. Có nó nghĩa là đã cắt MẤT chữ.
    assert.ok(!m.includes('[cắt bớt, bản đầy đủ dài'), 'truncateSafely() đã cắt -> mất đuôi mà vẫn đánh số như đủ');
    assert.equal(truncateSafely(m, 4000).daCat, false, 'truncateSafely trên phần này phải là no-op');
  }
});
...
test('I3 🔴 THROTTLE là thật: các tin cách nhau ≥ minKhoangCachMs', async () => {
  ...
  setThrottle({ moiTinMs: 0, dmMs: 0, minKhoangCachMs: 60, toiDaMoiPhut: 999 });
  await sendInParts(api, 'nhom1', RAT_DAI);
  resetThrottle();
...
test('I4 chạm trần số tin -> daCat = true và KÊU (thiếu chữ phải nói ra)', async () => {
  ...
  let kq;
  try {
    kq = await sendInParts(api, 'nhom1', 'x '.repeat(20000));   // 40k ký tự
  } finally {
    process.stderr.write = goc;
    resetThrottle();
  }
...
test('I5 needsSplitting: chỉ tin THẬT SỰ dài mới cần kênh phụ', () => {
  assert.equal(needsSplitting('ngắn thôi'), false);
  assert.equal(needsSplitting(RAT_DAI), true);
  assert.equal(needsSplitting(''), false);
});

test('I6 tin rỗng -> NÉM, không gửi tin trống', async () => {
  await assert.rejects(() => sendInParts(apiGia(), 'nhom1', '   \n  '), /RỖNG/);
});
...
test('I7 ★ một phần gửi HỎNG -> DỪNG NGAY, không bắn nốt phần sau', async () => {
  ...
  setThrottle({ moiTinMs: 0, dmMs: 0, minKhoangCachMs: 0, toiDaMoiPhut: 999 });
  await assert.rejects(() => sendInParts(api, 'nhom1', RAT_DAI));
  resetThrottle();
