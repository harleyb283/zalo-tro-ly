...
import {
  toId, toIdBatBuoc, cungId,
  datXuLyCanhBao, demCanhBao, datLaiDemCanhBao,
} from '../src/lib/ids.js';

/** Bắt cảnh báo thay vì để nó chạy ra stderr. */
const batCanhBao = [];
datXuLyCanhBao((c) => batCanhBao.push(c));
...
test('chuỗi dài — giữ nguyên, KHÔNG cảnh báo', () => {
  ...
  assert.equal(demCanhBao(), 0, 'chuỗi không được sinh cảnh báo nào');
});
...
test('🔴 số KHÔNG safe integer — PHẢI cảnh báo', () => {
  ...
  assert.equal(demCanhBao(), 1, 'phải cảnh báo đúng 1 lần');
...
test('toIdBatBuoc — ném lỗi khi thiếu, thông báo có tên trường', () => {
  chuanBi();
  assert.equal(toIdBatBuoc('123', 'chat_id'), '123');
  assert.throws(() => toIdBatBuoc(null, 'chat_id'), /chat_id/);
});

test('🔴 cungId — chống bẫy LỆCH KIỂU string vs number', () => {
  chuanBi();
  ...
  assert.equal(cungId('9990000000001', 9990000000001), true, 'cungId phải cứu được');
  assert.equal(cungId(null, null), false, 'hai cái null KHÔNG được coi là cùng ID');
  assert.equal(cungId('1', '2'), false);
});

test('bộ xử lý cảnh báo hỏng thì KHÔNG được làm chết luồng chính', () => {
  datXuLyCanhBao(() => { throw new Error('bộ xử lý hỏng'); });
