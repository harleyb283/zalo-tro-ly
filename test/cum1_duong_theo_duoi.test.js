...
import { confirmSchedule, dueSchedules, claimSending, createSchedule } from '../src/lich/schedule.js';
import {
  adjustCadence, isStillFollowingUp, parseCadence, closeFollowUp, dueFollowUps, stalledModelReminders,
  createFollowUp, modelWaitCapMs,
...
test('T1a ★★★ runOneTick chạy TRƯỚC vẫn KHÔNG đụng dòng la_theo_duoi=1', async () => {
  ...
  assert.deepEqual(dueSchedules(db, bayGio), [], 'dueSchedules vơ nhầm dòng theo đuổi');
...
test('T1a-2 ★★ claimSending TỰ NÓ từ chối dòng theo đuổi (lớp trong, chống ảnh tĩnh cũ)', () => {
  ...
  assert.equal(claimSending(db, doc(db, 'MOT1').id), true, 'lịch một lần phải nhận được');
  closeDb(db);
});
...
test('T1b ★★★ hai bộ chạy CÙNG TICK -> lời nhắc chỉ đi ĐÚNG MỘT tin', async () => {
  ...
  // Dòng #1 tới hạn SỚM HƠN nên `runOneTick` xử nó trước rồi chạm `await` —
  ...
  const a = runOneTick(chung);
  const b = runFollowUpTick({ ...chung, queryHistory, enqueueQuestion });
...
test('T1d-4 ★★ isStillFollowingUp: chặn host-đóng/tạm-dừng nhưng CHO QUA lượt chạm trần', () => {
...
test('T1e ★★★ adjustCadence({chuKyPhut:5}) -> mốc kế tiếp cách ĐÚNG 5 phút', () => {
  const db = dbTam();
  const t0 = Date.parse('2026-08-21T02:00:00Z');
...
test('T1f ★★ tham số `laBu` đã bỏ hẳn, và cả pack KHÔNG còn ternary hai nhánh giống nhau', () => {
  ...
  //     thích ở `_guiNhac`.
  const boChay = fs.readFileSync(path.join(GOC, 'src/lich/runner.js'), 'utf8');
  ...
  //     Mẫu này đã cắn hai lần trong một buổi (`index.js` CAN_QR và `runner.js` tienTo)
...
test('T1g ★★★ lượt CHẠM TRẦN vẫn phải GỬI (trần 3 = nhắc đủ 3 lần, không phải 2)', async () => {
  ...
  // trước khi gửi. `claimReminderTurn` lại ĐÓNG dòng ngay khi chạm trần rồi caller
  // mới gửi ⇒ bản đầu của lớp canh đã chặn mất chính lượt cuối, biến trần 3 thành
  // nhắc 2 lần. Bộ test cũ KHÔNG bắt được: H2 chỉ gọi `claimReminderTurn` trực tiếp,
