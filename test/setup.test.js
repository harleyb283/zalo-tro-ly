...
import { buildConfig, checkEnvironment, parseGroupChoice } from '../src/ops/setup.js';
...
test('A1 các kiểu gõ thường gặp đều hiểu đúng', () => {
  assert.deepEqual(parseGroupChoice('1,3', 5), { ok: true, chon: [0, 2] });
  assert.deepEqual(parseGroupChoice('1 3', 5), { ok: true, chon: [0, 2] });
  assert.deepEqual(parseGroupChoice('2-4', 5), { ok: true, chon: [1, 2, 3] });
  assert.deepEqual(parseGroupChoice(' 3 , 1 ', 5), { ok: true, chon: [0, 2] }, 'thừa khoảng trắng vẫn hiểu');
  assert.deepEqual(parseGroupChoice('2,2,2', 5), { ok: true, chon: [1] }, 'gõ trùng thì gộp');
});

test('A2 ★ bỏ trống = KHÔNG nhóm nào (⛔ không tự chọn hộ)', () => {
  assert.deepEqual(parseGroupChoice('', 5), { ok: true, chon: [] });
  assert.deepEqual(parseGroupChoice('   ', 5), { ok: true, chon: [] });
});
...
test('A3 ★★ gõ sai thì BÁO để hỏi lại, ⛔ TUYỆT ĐỐI không đoán bừa', () => {
  ...
  for (const xau of ['abc', '1,abc', '0', '9', '1-9', '-2']) {
    const kq = parseGroupChoice(xau, 5);
...
test('A4 "tất cả" hiểu được, nhưng đó là việc của chỗ gọi phải hỏi lại', () => {
  assert.deepEqual(parseGroupChoice('tất cả', 3), { ok: true, chon: [0, 1, 2] });
  assert.deepEqual(parseGroupChoice('all', 3), { ok: true, chon: [0, 1, 2] });
});
...
test('B1 ★★★ dmChatId phải được điền SẴN bằng chính userId', () => {
  ...
  // ai hiểu vì sao.
  const c = buildConfig({ mau: MAU, toi: { userId: '777', ten: 'Chị A' }, nhomChon: [] });
...
test('B2 ★ giữ nguyên mọi khoá khác của bản mẫu', () => {
  const c = buildConfig({ mau: MAU, toi: { userId: '777' }, nhomChon: [] });
...
test('B4 nhóm chưa đặt tên vẫn có nhãn đọc được', () => {
  const c = buildConfig({ mau: MAU, toi: { userId: '777' }, nhomChon: [{ chatId: '222', ten: null }] });
  assert.equal(c.groups[0].ten, 'nhóm 222');
});

test('B5 ★ thiếu userId -> NÉM, ⛔ không ghi ra file cấu hình hỏng', () => {
  assert.throws(() => buildConfig({ mau: MAU, toi: {}, nhomChon: [] }), /userId/);
});
...
test('C1 ★ Node quá cũ -> nêu rõ phải làm gì', () => {
  const kq = checkEnvironment({ phienBanNode: 'v18.0.0', coNodeModules: true });
...
test('C2 ★ chưa npm install -> đưa đúng câu lệnh cần gõ', () => {
  const kq = checkEnvironment({ phienBanNode: 'v22.0.0', coNodeModules: false });
  assert.equal(kq.ok, false);
  assert.equal(kq.van[0].cach, 'Gõ: npm install');
});

test('C3 máy đủ điều kiện -> ok, ⛔ không kêu ca gì', () => {
  const kq = checkEnvironment({ phienBanNode: 'v26.7.0', coNodeModules: true });
